import cv2
from .testpymongo import addExerciceWithNameUser
import threading
    ...
    def updateActivity(self,name):
        ...
        if name=="nothing":
            ...
            if self.currentActivity is not None :
                # ici j'ajoute l'activity mais ça sert pas à grand chose
                self.listActivity.append(self.currentActivity)
                self.currentActivity=None
                # ici j'ajoute l'exercice dans mongoDB
                t = threading.Thread(target=addExerciceWithNameUser, args=(self.name,self.currentActivity.name,self.currentActivity.time,))
                t.start()
            print(self.nothingActivity.time)
