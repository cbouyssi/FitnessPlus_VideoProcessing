...
from src.utils.structure import PersonActivities, Machine,Activity
...
from src.utils.monitoringSalleandUserActivity import updateTimeMachine, printMachineState,usedMachine
...
def model_prediction_with_Person(score_threshold=0.3,iou_threshold=0.2):
    ...
    test_path = os.path.expanduser('data/frames_vid_salle_insa_cut')
    output_path = os.path.expanduser('data/frames_vid_salle_insa_160_with_PersonActivityn_out')
    ...
    # for image_file in os.listdir(test_path):
        ...
        for i, c in reversed(list(enumerate(out_classes))):
            if c==0 :
                ...
                rgbHist=[]
                listActivity=[]
                # listActivity.append(Activity("traction",10))

                if comp==0 :


                    rgbHist.append(histr)
                    person=PersonActivities("person"+ str(id),comp,box,rgbHist,0,listActivity)
                ...
                if comp  % 1== 0: #faut mettre 30 pour de vrai
                    machines,person = updateTimeMachine(person, machines)
                    persons[person.name]=person
        ...
        out_user=open(os.path.expanduser("out_user.txt"),"w+")
        for k,person in persons.items():
            out_user.write("user\n")
            out_user.write("id:"+person.name+"\n")
            out_user.write(person.nothingActivity.name+":"+str(person.nothingActivity.time)+"\n")
            for activity in person.listActivity :
                out_user.write(activity.name+":"+str(activity.time)+"\n")
        out_user.close()
